...
def add_sleep_schedule(
	...
	db = get_db()
	cursor = db.execute(
		"INSERT INTO sleep_schedule (days, hour, minute, duration) VALUES (?, ?, ?, ?)",
		("^".join("1" if d else "0" for d in days), hour, minute, duration),
	)
	db.commit()
...
def update_sleep_schedule(
	...
	db = get_db()
	db.execute(
		"UPDATE sleep_schedule SET days = ?, hour = ?, minute = ?, duration = ? WHERE id = ?",
		("^".join("1" if d else "0" for d in days), hour, minute, duration, id),
	)
	db.commit()
